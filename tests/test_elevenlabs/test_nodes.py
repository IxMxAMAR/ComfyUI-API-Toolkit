"""Validation tests for ElevenLabs nodes."""

import pytest
from services.elevenlabs.nodes import NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS


EXPECTED_NODE_COUNT = 15
CATEGORY_PREFIX = "API Toolkit/ElevenLabs"

# Auth node
AUTH_NODE_NAMES = ["AIS_EL_APIKey"]

# Config/selector nodes that do NOT call external APIs
CONFIG_NODE_NAMES = {
    "AIS_EL_APIKey",
    "AIS_EL_VoiceSelector",
}


class TestElevenLabsNodeCount:
    def test_expected_count(self):
        assert len(NODE_CLASS_MAPPINGS) == EXPECTED_NODE_COUNT


class TestElevenLabsNodeAttributes:
    """Every ElevenLabs node must have required ComfyUI attributes."""
...
class TestElevenLabsAuthPassword:
